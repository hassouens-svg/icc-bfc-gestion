...
const ContactGroupsSMSPage = () => {
    ...
    lines.forEach((line, index) => {
      ...
      if (phoneMatch) {
        ...
        if (phoneClean.length >= 8) {
          ...
          contacts.push({ prenom, nom, email: '', telephone: phoneClean });
        }
      }
    });
  ...
  const handleDelete = async (id, name) => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/contact-groups-sms/${id}`,
        {
          method: 'DELETE',
          headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
        }
      ...
      <div className="p-6 max-w-5xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <h1 className="text-3xl font-bold">📦 Boxes SMS</h1>
            </div>
            <p className="text-gray-600">Créez des groupes de numéros réutilisables pour vos envois SMS</p>
          </div>
        ...
        {showCreate && (
          <div className="bg-white rounded-lg shadow p-6 mb-6">
            <h2 className="text-xl font-semibold mb-4">Créer une Box SMS</h2>
            ...
            <div className="space-y-4">
              ...
              <div>
                <Label>Contacts (Prénom Nom numéro) *</Label>
                <Textarea
                  value={newGroup.contacts_text}
                  onChange={(e) => setNewGroup({...newGroup, contacts_text: e.target.value})}
                  placeholder="Format: Prénom Nom 0612345678 (un par ligne)&#10;&#10;Jean Dupont 0612345678&#10;Marie Martin +33687654321&#10;Paul Dubois 0698765432"
                  rows={8}
...
export default ContactGroupsSMSPage;
